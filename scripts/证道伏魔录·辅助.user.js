...
(() => {
    ...
    const addOneCloseButton = () => {
        ...
        let buttonGroup = document.querySelector("#ajaxHtmlShow > div:nth-child(4) > div > div.col-md-12.col-xs-12 > div:nth-child(4)")
        if (title && title.innerText === '外事堂' && !document.querySelector("#sp_for")) {
            // 如果存在说明是在秘境中
            if (buttonGroup) {
                return false
            }
            ...
            button.onclick = () => {
                ...
                if (confirm('您确定要进行一键关闭吗？')) {
                    document.querySelectorAll(".text-nowrap")[1].querySelectorAll(".button9").forEach(e => e.click())
    ...
    const addOneSignButton = () => {
        ...
        const menuElement = document.querySelector("#ajaxHtmlShow > .body_md.pbgb  div.row > div:nth-child(4) > div");
        if (menuElement && !document.querySelector("#sign")) {
            ...
            buttonElement.onclick = () => {
                Promise.all([getVip(), getSign(), getWj()]).then(() => {
    ...
    const addBatchCleanBtn = () => {
        ...
        if (title && title.innerText.includes("天地棋局") && !document.querySelector("#batchClean")) {
    ...
    const addUpAllByPartner = () => {
        ...
        let title = document.querySelector("#ajaxHtmlShow > div.body_md.pbgb > div > h4")
        let partners = document.querySelectorAll(".button_tz_get")
        let buttonGroup = document.querySelector("#ajaxHtmlShow div > div.col-md-12.col-xs-12 > div:nth-child(2)")
        if (title && title.innerText.includes("伙伴列表") && partners.length > 0 && !document.querySelector("#upByPartner")) {
            ...
            button.onclick = () => {
                Promise.all(Array.from(partners).map(partner => upByPartner(partner.name, partner.parentNode.parentNode.innerText.replace(/\n/, " "))))
    ...
    const addJoinToKill = () => {
        ...
        const title = document.querySelector("#ajaxHtmlShow > div.body_md.pbgb > div > div.col-md-12.col-xs-12 > h4.panel-title.col-md-12.col-xs-12");
        if (title && title.innerText.includes("天魔入侵") && !document.querySelector("#joinToKill")) {
            let button = document.createElement("input")
            button.className = "button6"
            button.type = "button"
            button.value = "一键入队"
            ...
            button.onclick = () => {
                ...
                Promise.all(teamIndexs.map((i) => getTeamId(i).then((teamid) => join(teamid))))
                    .then(() => alert("一键入队执行成功"))
    ...
    const addOneClickCultivate = () => {
        ...
        const menuElement = document.querySelector("#ajaxHtmlShow > div.body_md.pbgb > div > div > div.col-md-12.col-xs-12");
        const titleElement = document.querySelector("#ajaxHtmlShow > div.body_md.pbgb > div > div > h4:nth-child(1)");

        if (titleElement && titleElement.innerText.includes("灵植仓库") && menuElement && !document.querySelector("#oneClickCultivate")) {
            const buttonElement = document.createElement("input");
            buttonElement.type = "button";
    ...
    const addUpgradeFarmland = () => {
        ...
        if (titleElement && titleElement.innerText.includes("灵植仓库") && menuElement && !document.querySelector("#oneClickUpgrade")) {
            const buttonElement = document.createElement("input");
            buttonElement.type = "button";
            buttonElement.className = "button3 submit";
    ...
    const addRefineEquipmentButton = () => {
        ...
        const targetElement = document.querySelector("#ajaxHtmlShow > div.body_md.pbgb > div > div.col-md-12.col-xs-12.lazydiv");
        const h4Element = document.querySelector("#ajaxHtmlShow > div.body_md.pbgb > div > div.col-md-12.col-xs-12.lazydiv > h4");

        if (h4Element && h4Element.innerText.includes("装备强化") && !document.querySelector("#oneClickRefine")) {
            const buttonElement = document.createElement("input");
            buttonElement.type = "button";
