...
(function () {
  ...
  if (ulButton) {
    try {
      ...
      let html =
        '<li class="reply ac_show" id="action"><span>随机回复</span></li>';
  ...
  function getHitokoto() {
    ...
    httpRequest.onload = function () {
      if (httpRequest.readyState == 4 && httpRequest.status == 200) {
        ...
        alert(
          `HTTP请求失败,错误原因: 请求的URL的相应状态=${httpRequest.status}\n请求的结果状态=${httpRequest.readyState}`,
        );
      }
    };
  ...
  function registerMenuCommand() {
    ...
    menu_enable_toggle_ID = GM_registerMenuCommand(
      `🔄 ${enable ? "开启" : "关闭"} 自动回复 - 点击切换`,
      menu_enable_toggle,
    );
  ...
  function menu_Quick() {
    ...
    if (!box) {
      let quickHTML =
        '<div id="box" style="position: fixed; top: 105px; left: 1022px; padding: 10px; background: rgb(255, 255, 255) none repeat scroll 0% 0%; border-radius: 10px; z-index: 9; cursor: default; border: 1px solid;"><input type="button" value="X" id="close" style="border: 1px solid;background: rgba(255, 255, 255,0) none repeat scroll 0% 0%;padding: 0;float: right;width: 20px;border-radius: 5px;text-align: center;"><h1>快捷评论管理</h1><textarea id="text" autofocus="true" style="width: 250px;height: 200px;border-radius: 5px;border: 1px solid;"></textarea><br><input type="button" value="重置" id="reset" style="border: none;background: rgba(255, 255, 255,0) none repeat scroll 0% 0%;padding: 0;margin: 0;">&nbsp;&nbsp;<input type="button" value="保存" id="save" style="border: none;background: rgba(255, 255, 255,0) none repeat scroll 0% 0%;padding: 0;margin: 0;"></div>';
